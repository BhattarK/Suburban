...
def update_block_data_sta(record_data, connection: pyodbc.Connection):
    ...
    for document_id, blocks in block_data.items():
         with connection.cursor() as cursor:
                    # cursor.execute(
                    #     """UPDATE document_line b
                    #         SET transaction_code_rec_type = 'PROPANE-AIR METER'
                    #         FROM document_line a
                    #         WHERE a.documentid  = b.documentid
                    #         AND b.documentid = ?
                    #         and b.transaction_code_rec_type = 'PROPANE METER'
                    #         and exists (select * from document_line c where c.documentid = a.documentid 
                    #                 and c.recordtype = '17' and c.pressure_altitude_conversion in ('5550','','5610','7060'))""",
                    #     document_id 
                    # )  
         #with connection.cursor() as cursor:              
                    cursor.execute(
                        """UPDATE document_line 
                            SET step_rates = replace(step_rates,E'\n','<br>')
                            where                            
                            documentid = ?
                             and step_rates not in ('0')""",
                        document_id 
                    )                  
    for document_id, blocks in block_data.items():
        ...
        for block in blocks:
            ...
            if found_18  and previous_document_id!= document_id:   
                #print(document_id)                 
                ...
                if  len(address_count)  > 1 :                       
                      ...
                      with connection.cursor() as cursor:
                            ...
                            min_row_number = getattr(record, "minrownumber")
                            max_row_number = 0     
                            orig_min_row_number = min_row_number                       
                            counter = 0
                            ...
                            #transaction_invoice_comment_exists = False
                            for record in cursor.fetchall():
                                document_lineids[getattr(record, "document_lineid")] = [getattr(record, "recordtype"), getattr(record, "price_prot_gallons_remaining")]   
                                #document_lineids[getattr(record, "document_lineid")] =  getattr(record, "price_prot_gallons_remaining")  
                            with connection.cursor() as cursor:        
                                for index,document_lineid in enumerate(document_lineids.keys()):
                                    print(document_lineid)
                                    if document_lineids[document_lineid][0] =="13" and document_lineids[document_lineid][1]:
                                        print(document_lineids[document_lineid][1])
                                        orig_min_row_number = orig_min_row_number - 1
                                        cursor.execute(
                                                "UPDATE document_line SET rownumber = ? WHERE document_lineid = ?",
                                                orig_min_row_number,
                                                document_lineid
    ...
    conversioncodes = ["5550","5610","7060"]           
    for document_id, blocks in block_data.items():
        with connection.cursor() as cursor:                            
                    cursor.execute(
                        """UPDATE document_line 
        ...
        with connection.cursor() as cursor: 
            cursor.execute(
                            """ SELECT 
                                ...
                                ,dl.recordtype
                                ,dl.rownumber
                                ,dl.pressure_altitude_conversion
                                ,dl.document_lineid
                                ,dl.transaction_code_rec_type as transaction_code
            ...
            for record in cursor.fetchall():
                    #address_count.append(getattr(record, "address_count"))  
                    document_lineid =   getattr(record, "document_lineid")
                    pressure_altitude_conversion =       getattr(record, "pressure_altitude_conversion")
                    recordtype =  getattr(record, "recordtype")
                    transaction_code = getattr(record, "transaction_code")
                    customer_account_number = getattr(record, "customer_account_number")
                    if "8905" in customer_account_number: 
                        print("A")
                    if recordtype == '12' and transaction_code == "PROPANE METER": 
                       line_id_to_update =  document_lineid                       
                    
                    if recordtype == '17' and str(pressure_altitude_conversion) in conversioncodes:                        
                            print("A")
                            cursor.execute(
                            """UPDATE document_line 
                                SET transaction_code_rec_type = 'PROPANE-AIR METER'
                                WHERE document_lineid = ? """,
                            line_id_to_update 
                        )  
