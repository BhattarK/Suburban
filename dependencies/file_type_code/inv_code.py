...
def update_block_data_inv(record_data, connection: pyodbc.Connection):
    ...
    for document_id, records in record_data.items():
        ...
        # for line in (propane_air_docids): 
        #     print(line)               
    ...
    for document_id, blocks in block_data.items():
        ...
        if multiple_12 == False: 
            #print(sub_total_addends)
            with connection.cursor() as cursor:
                cursor.execute(
                    "UPDATE document_line SET sub_total = ? WHERE documentid = ? and recordtype = '17'",
                    grand_total,
                    document_id
                )                
                ...
                # if count > 0: 
                #    cursor.execute(
                #     "UPDATE document_line SET sub_total = null WHERE documentid = ? and recordtype = '12'",                    
                #     document_id
                # )  
        ...
        with connection.cursor() as cursor: 
            ...
            for record in cursor.fetchall():
                    ...
                    if recordtype == '17' and str(pressure_altitude_conversion) in conversioncodes:                        
                            ...
                            cursor.execute(
                            """UPDATE document_line 
                                SET transaction_code = 'PROPANE-AIR METER'
