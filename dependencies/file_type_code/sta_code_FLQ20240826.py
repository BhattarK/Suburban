...
def update_block_data_sta(record_data, connection: pyodbc.Connection):
    ...
    for document_id, blocks in block_data.items():
        ...
        for block in blocks:
                ...
                if  len(address_count) > 1 :                       
                       ...
                       for document in document_lineids_12blokcs.values():                            
                                ...
                                for ind, column in enumerate(df.columns):
                                    ...
                                    sub_total = sub_total + transaction_dollars
                                    rownumbers.append(rownumber)
                                    if transaction_code == "PROPANE" or  transaction_gallons >0: 
                                        #print("A")
                                        if key18s[counter]: 
                                            #print("A")
                                            with connection.cursor() as cursor:                                                                                                           
            ...
            #     with connection.cursor() as cursor:  
            #         cursor.execute(
            #             "DELETE FROM document_line WHERE documentid = ? AND recordtype = '18' and transaction_reference = ?",                                            
            #             document_id, 
            #             record18_trans_ref
            #         )  
            # elif found_18  and previous_document_id== document_id:       
            #     print("A") 
            #     # with connection.cursor() as cursor:                                                                                                           
            #     #     cursor.execute(
