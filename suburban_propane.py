...
def get_input_filenames(connection):
    ...
    #      r"INPUT\ARCHIVE\DECODED\E3_240426_SAEINV.TXT",   
    ...
    #       #r"INPUT\DECODED\E3_240330_SAEINV.TXT"               
    #       #r"INPUT\DECODED\E3_240401_SAESTA.TXT",
    #],key = os.path.getsize)
...
def process_input_file(input_filename, connection):
    ...
    with open(input_filename, "r+") as input_file:
        ...
        for line_number, line in enumerate(input_file.readlines()):
            ...
            if line[:2] == "11" or (True if "LETTERS" in input_filename and line[:2] == "D0" else False):
                document_count += 1
        if "denied_credit" not in input_filename:
            invoices.append(current_invoice)
    ...
    for invoice in invoices:
        ...
        with connection.cursor() as cursor:
            ...
            for secondary_query_data in secondary_table_query_data:
                ...
                if "STA" in input_filename or  "INV" in input_filename :
...
def main():
    ...
    for input_filename in input_filenames:
        try:
            if ("SAEINV" in input_filename or "SAESTA" in input_filename or "SAEREN" in input_filename or "SAEDUN" in input_filename or "LETTERS" in input_filename or "denied_credit" in input_filename):
                display_message(f"Processing {os.path.basename(input_filename)}")
                with connection.cursor() as cursor:                   
                   cursor.execute(f"SELECT datafilename FROM DATAFILE WHERE datafilename = '{os.path.basename(input_filename)}'")
                   datafilename = ""
                   for record in cursor.fetchall():
                       #branchid = getattr(record, "datafilename")
                       datafilename = getattr(record, "datafilename")
                ...
                #os.system(rf"python D:\Web\Online\Suburban-sandbox\Process\inserts.py {os.path.basename(input_filename)}")
                os.system(rf"python D:\Web\Online\Suburban\Process\inserts.py {os.path.basename(input_filename)}")
                ...
                if "denied_credit" not in input_filename:
                    ...
                    if  "SAEINV" in input_filename: 
                        ...
                        header_for = "STATEMENT"
                    elif "SAEDUN" in input_filename: 
                        header_for = "DUNNING"   
                    elif "LETTERS" in input_filename: 
                          header_for = "LETTERS"
                    elif "SEAREN" in input_filename: 
                          header_for = "RENTALS"      
                    ...
                    file_date = datetime.strptime(file_date, "%Y/%m/%d").strftime("%m/%d/%Y")
                    if "LETTERS" in input_filename: 
                        document_date = file_date
                    else: 
                        document_date = datetime.strptime(document_date, "%Y-%m-%d").strftime("%m/%d/%Y")                  
                    
                    
                    report_header = """\n\n#######################################################################################\n
  {} REPORT OF COUNTS FOR RUN TIME {} EST PROCESSING DATE IS {}
                                   \n#######################################################################################
                                   \n{} is {} Date For {} COUNTS FROM {}\n
 ______________________________________________________________________________________\n
{}\n 
                                 """.format(header_for,curr_time,curr_date,document_date,header_for,header_for,os.path.basename(input_filename),file_data)
                                 
                    report_footer = """\n#######################################################################################\n
  THE TOTAL # OF RECORDS FOR {} = {}\n
#######################################################################################\n\n                                   
                                 """.format(os.path.basename(input_filename), document_count)
                                 
                    with open(file_data_filename, "w+") as file_data_file:
                        file_data_file.write(report_header)  
                        #file_data_file.write("\n") 
                        #file_data_file.write(file_data)
                        file_data_file.write(report_footer)  
                        email_body +=  """{}\n
                                          {}      
                ...
                send_email(
                    ...
                    file_data_filename if "denied_credit" not in input_filename else None
                )
                ...
                for pdf in os.listdir(r"D:\PDF\Production\010222024"):
                    pdf = os.path.join(r"D:\PDF\Production\010222024", pdf)
                    if not os.path.isdir(pdf):
                        if os.path.exists(os.path.join(r"D:\PDF\Production", os.path.basename(pdf))):
                            os.remove(os.path.join(r"D:\PDF\Production", os.path.basename(pdf)))
                        shutil.copy(pdf, r"D:\PDF\Production")
                        if os.path.exists(os.path.join(r"D:\PDF\Production\010222024\done", os.path.basename(pdf))):
                           os.remove(os.path.join(r"D:\PDF\Production\010222024\done", os.path.basename(pdf)))
                        shutil.move(pdf, r"D:\PDF\Production\010222024\done")
...
if __name__ == "__main__":
    ...
    try:
        ...
        combined_string = ""   
        with open(log_filename,"r") as fileread: 
              combined_string = ''.join(fileread.readlines()).replace('\n', '')
        send_error_email(error,body=combined_string)
    with open("sequence_number.txt", "w+") as sequence_number_file:
