...
def remove_substring(string, start_index, end_index):
  """Removes a substring from a given string.

  Args:
    string: The original string.
    start_index: The starting index of the substring (inclusive).
    end_index: The ending index of the substring (inclusive).
        ...
        for line in Lines: 
            if line.startswith("D"): 
                ...
                if len(check_month[0]) ==1:                    
                    ...
                    line = line.replace(str(df["check_date"].values[0]).rstrip(),"0" + str(df["check_date"].values[0]))
                    print(dateindex)
                    line = insert_string(line, dateindex-1, "0" + str(df["check_date"].values[0]))                
                    line = remove_substring(line, 235, 244)    
                    line = insert_string(line, 246, " ") 
            myfile.write(line)      
    ofile.close()                                
